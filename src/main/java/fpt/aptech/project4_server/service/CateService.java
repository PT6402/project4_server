...
public class CateService {
    ...
    // public ResponseEntity<ResultDto<?>> UpdateCate(Integer id, CateAdCreateRes
    // cateres) {
    // try {
    // Optional<Category> cateUp = caterepo.findById(id);
    //
    // if (cateUp.isEmpty()) {
    // ResultDto<?> response = ResultDto.builder().status(false).message("Category
    // is not existed").build();
    ...
    //
    // ResultDto<?> response = ResultDto.builder().status(true).message("Update
    // successfully").build();
    // return new ResponseEntity<ResultDto<?>>(response, HttpStatus.OK);
    // } catch (Exception e) {
    // System.out.println(e.getMessage());
    // ResultDto<?> response = ResultDto.builder().status(false).message("Update
    // fail").build();
    // return new ResponseEntity<ResultDto<?>>(response, HttpStatus.BAD_REQUEST);
    // }
    // }
    ...
    public ResponseEntity<ResultDto<?>> UpdateCate(Integer id, CateAdCreateRes cateres) {
        try {
            ...
            // Check if the new name is different from the current name before checking for
            // conflicts
            if (!existingCategory.getName().equals(cateres.getName())) {
                var listcheck = caterepo.findAll();
                for (Category c : listcheck) {
                    if (c.getName().equals(cateres.getName())) {
                        ResultDto<?> response = ResultDto.builder().status(false).message("Category name is existed")
                                .build();
    ...
    public ResponseEntity<ResultDto<?>> CateUserShow() {
        try {
            var listcate = caterepo.findAll().stream().map(c -> CateUserRes.builder().id(c.getId()).name(c.getName())
                    .description(c.getDescription()).Imagedata(c.getImagedata()).build());
